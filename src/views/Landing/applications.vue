<template>
  <section class="w-full h-screen bg-[#123524] py-2 px-4 flex   items-center justify-center">
    <div class="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-center">
      ...
      <div class="text-white h-full flex flex-col justify-between">
        ...
        <div class="flex flex-col gap-3 text-sm sm:text-base">
          <a href="tel:+998712000000" class="flex items-center gap-3 text-white/90 hover:text-white transition-colors">
            <span class="text-white/50">📞</span> +998 71 200 00 00
          </a>
          <a href="/src/components/Landing/ContactInfo.vue"
      ...
      <div class="bg-white rounded-2xl shadow-xl p-5 sm:p-7">
        ...
        <div v-if="submitted" class="flex flex-col items-center justify-center text-center py-5 px-2">
        ...
        <template v-else>
          ...
          <div class="flex items-center mb-6 overflow-x-auto">
            <template v-for="(step, i) in steps" :key="step.label">
              <div class="flex flex-col items-center shrink-0">
                <div
                  ...
                  :class="i + 1 < currentStep
                    ...
                    ">
                  <svg v-if="i + 1 < currentStep" class="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor" stroke-width="3">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                  </svg>
                  <span v-else>{{ i + 1 }}</span>
                </div>
                <span class="mt-1.5 text-[10px] sm:text-xs whitespace-nowrap" :class="i + 1 === currentStep
              ...
              <div v-if="i < steps.length - 1" class="flex-1 h-px mx-1.5 sm:mx-2 mb-4 transition-colors duration-200"
          ...
          <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
            <transition name="fade-slide" mode="out-in">
              ...
              <div v-if="currentStep === 1" key="step1" class="flex flex-col gap-4">
                <div>
                  ...
                  <input v-model="form.name" type="text" placeholder="Masalan: Baraka Restoran"
                    class="w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm text-black placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500" />
                </div>
              ...
              <div v-else-if="currentStep === 2" key="step2" class="flex flex-col gap-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input v-model="form.instagram" type="text" placeholder="@biznes_uz"
                      class="w-full rounded-lg border text-slate-800 border-gray-200 px-3.5 py-2.5 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500" />
                  </div>
                  <div>
                    <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5">
                    ...
                    <input v-model="form.telegram" type="text" placeholder="t.me/biznes_uz"
                      class="w-full rounded-lg border text-slate-800 border-gray-200 px-3.5 py-2.5 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500" />
                  </div>
                </div>
              ...
              <div v-else key="step4" class="flex flex-col gap-4">
                <div>
                  <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-2.5">
                    Chegirma foizi *
                  </label>
                  <div class="grid grid-cols-4 gap-2">
                    <button v-for="option in discountOptions" :key="option" type="button"
                      class="rounded-full py-2 text-sm font-semibold border transition-colors" :class="form.discount === option
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input v-model="form.minAmount" type="number" inputmode="numeric" placeholder="50 000"
                      class="w-full rounded-lg border text-slate-800 border-gray-200 px-3.5 py-2.5 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500" />
                  </div>
                  <div>
                    <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5">
                ...
                <div class="flex items-start gap-2.5 rounded-lg bg-emerald-50 border border-emerald-100 px-3.5 py-3">
            ...
            <div class="flex items-center justify-between mt-2">
              ...
              <button v-if="currentStep > 1" type="button"
                class="text-sm text-gray-500 hover:text-gray-700 transition-colors" @click="prevStep">
                ← Orqaga
              </button>
            ...
            <span v-if="currentStep > 1" class="text-xs text-gray-400 text-right -mt-2">
              {{ currentStep }}/{{ steps.length }} qadam
            </span>
          </form>
        </template>
